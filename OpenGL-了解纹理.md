...
我们知道无论在游戏中还是一些图形绘制中都不可能是单纯的颜色来进行着色渲染，会使用一些图片纹理进行贴图，提高画面的真实度。纹理贴图就是把绘制着色的部分替换成了纹理进行填充，所以还是很好理解纹理这个概念。我们都知道图片的大小为 **宽x高x8x3** 字节。在OpenGL中纹理一般采用.tga格式文件，这种格式文件是一种特殊格式文件，在OpenGL ES中还可以采用位图png、jpg等格式文件。可以查看[这篇文章](https://juejin.im/post/5d29987c51882568f83c8afe)关于图片的编码。
